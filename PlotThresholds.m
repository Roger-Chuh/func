...
for i = 1 : length(pids)
    for j = 1 : 4
        Data{i, j} = cell(length(timestamps), 1);
    end
end
for i = 1 : size(a, 1)
    %Data{i,1}
    ...
    Data{pid_index,  data.target_cid}{time_index} = data;
end
...
for pid = 1 : size(Data,1)
    ...
    for cid = 1 : size(data,2)
        ...
        if cnt > 2
            figure(1),clf;subplot(3,2,1); %legend('thr', 'calc');title('disp');
...
%             Angle(Angle < 0) = 180 + Angle(Angle < 0);
            ...
            if 1
                figure(100),clf;subplot(2,2,1);plot([traces_2d(traces_predict_2d(:,1) < 1000,1) traces_predict_2d(traces_predict_2d(:,1) < 1000,1)]);title('x'); 
