function testFisheyeRadTanThinPrism()
...
if 0
    ...
    if 0
        ...
        d_udvd_d_fxfycxcy = [-(u-cx)./fx./fx           0             -1./fx     0;
                                     0          -(v-cy)./fy./fy    0      -1./fy];
    end
end
d_udvd_d_uv = [1./fx      0;
...
function d_err_d_pt3d =  computeBearing(transedXYZ_c)
ptNorm = norm(transedXYZ_c);
    ptNorm_3_2 = -0.5 * (1 / (ptNorm * ptNorm * ptNorm));
     d_err_d_pt3d = eye(3);
    d_err_d_pt3d(0+1, 0+1) = 1.0 / ptNorm + transedXYZ_c(0+1) * (ptNorm_3_2 * 2 * transedXYZ_c(0+1));
    d_err_d_pt3d(0+1, 1+1) = transedXYZ_c(0+1) * (ptNorm_3_2 * 2 * transedXYZ_c(1+1));
    d_err_d_pt3d(0+1, 2+1) = transedXYZ_c(0+1) * (ptNorm_3_2 * 2 * transedXYZ_c(2+1));

    d_err_d_pt3d(1+1, 0+1) = transedXYZ_c(1+1) * (ptNorm_3_2 * 2 * transedXYZ_c(0+1));
    d_err_d_pt3d(1+1, 1+1) = 1.0 / ptNorm + transedXYZ_c(1+1) * (ptNorm_3_2 * 2 * transedXYZ_c(1+1));
    d_err_d_pt3d(1+1, 2+1) = transedXYZ_c(1+1) * (ptNorm_3_2 * 2 * transedXYZ_c(2+1));

    d_err_d_pt3d(2+1, 0+1) = transedXYZ_c(2+1) * (ptNorm_3_2 * 2 * transedXYZ_c(0+1));
    d_err_d_pt3d(2+1, 1+1) = transedXYZ_c(2+1) * (ptNorm_3_2 * 2 * transedXYZ_c(1+1));
    d_err_d_pt3d(2+1, 2+1) = 1.0 / ptNorm + transedXYZ_c(2+1) * (ptNorm_3_2 * 2 * transedXYZ_c(2+1));
end
