function stereoParam = CalibCubeStereo(paramLeft, paramRight, cbcXYLeft, cbcXYRight, cbGrid, ...
...
cbcXYLeft0 = cbcXYLeft;
cbcXYRight0 = cbcXYRight;
...
rotVecRef = median(rotVecRef, 2); %  + 0.05*(rand(3,1)-0.5);
...
while ((change > 5e-6) && (iter <= maxIter))
    ...
    for iImgPair = 1:nImgPair
        ...
        cbcXYLeft{iImgPair} = cbcXYLeft0{iImgPair}(1:2,:);
        cbcXYRight{iImgPair} = cbcXYRight0{iImgPair}(1:2,:);
        if 0
            nPt = size(cbGrid{iImgPair}, 2);
        else
            nPt = size(cbcXYLeft{iImgPair}, 2);
        end
        ...
        if 0
            ...
            cubeIdx = cbGrid{iImgPair}(:,1);
            %构建左相机的三面的雅可比残差
            [xl,dxldrl,dxldtl,dxldfl,dxldcl,dxldkl,dxldalphal] = project_points_cube_flow(iImgPair, cbGrid, rotVecLeft,transVecLeft,focLeft,cenLeft,kcLeft,alphaLeft,cubeIdx,configLeft);
        ...
        if 0
            ...
            %构建右相机的三面的雅可比残差
            [xr,dxrdrr,dxrdtr,dxrdfr,dxrdcr,dxrdkr,dxrdalphar] = project_points_cube_flow(iImgPair, cbGrid, rotVecRight,transVecRight,focRight,cenRight,kcRight,alphaRight,cubeIdx,configRight);
        end
