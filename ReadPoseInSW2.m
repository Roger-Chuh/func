...
data(:,[1 2 11]) = data(:,[1 2 11]) - data(1,11);

data_orig = data;

data_nf = data(data(:,3) == 0,:);
...
Data_nf = data_nf(:,4:10);
poseMat_nf = [];
for i = 1 : size(Data_nf,1)
    data1 = Data_nf(i,:);
    xyzw = data1(4:7);
    trans = data1(1:3);
    ...
    %     R2 = quatern2rotMat(xyzw([4 1 2 3]));
    R = quat2rotm(xyzw([4 1 2 3]));
    poseMat_nf = [poseMat_nf; [reshape(R,1,9), trans]];
    %     rotm2quat(R)
    %     rotMat2quatern(R2)
end
