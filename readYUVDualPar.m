function readYUVDualPar( inputDir , width, height,scale)
...
idList = [1:frameNum 1:frameNum]';

dirInfoCat = [dirInfo];
inputDirCat = [repmat({inputDir}, 2*frameNum,1)];

whichCat = [repmat({'L'}, frameNum,1); repmat({'R'}, frameNum,1);];
% for i 
if ~nonYUV
    if cfg.coreNum > 0
        parfor i =    1 : 2*frameNum  %[1 2 4 6 8 13 14 16 18 20 25 26 28 30 35 36 37 39 41 47 48 49 51 53 55 58 60 61 63 65 67]
            
            readFile(inputDirCat{i}, dirInfoCat(i).name,  idList(i), width, height,colRng1,colRng2, patternNum, blurLevel, scale, whichCat{i});
            
            
        end
    else
        for i =    1 : 2*frameNum  %[1 2 4 6 8 13 14 16 18 20 25 26 28 30 35 36 37 39 41 47 48 49 51 53 55 58 60 61 63 65 67]
    ...
    if cfg.coreNum > 0
        parfor i =    1 : 2*frameNum  %[1 2 4 6 8 13 14 16 18 20 25 26 28 30 35 36 37 39 41 47 48 49 51 53 55 58 60 61 63 65 67]
            ...
            renameFile(inputDirCat{i}, dirInfoCat(i).name,  idList(i), width, height,colRng1,colRng2, patternNum, blurLevel, scale, whichCat{i});
