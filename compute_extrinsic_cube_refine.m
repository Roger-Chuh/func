function [omckk,Tckk,Rckk,JJ] = compute_extrinsic_cube_refine(omc_init,Tc_init,x_kk,X_kk,fc,cc,kc,alpha_c,MaxIter,thresh_cond, cubeIdx, config, iImg, cbGrid),
...
while (change > 1e-10)&(iter < MaxIter),
    ...
    %fprintf(1,'%d...',iter+1);
    if 1
%         [x,dxdom,dxdT] = project_points_cube2(X_kk,omckk,Tckk,fc,cc,kc,alpha_c,cubeIdx, config);
        [x,dxdom,dxdT] = project_points_cube_flow(iImg, cbGrid,omckk,Tckk,fc,cc,kc,alpha_c,cubeIdx, config);
        ex = x_kk - x;
    else
        for j = 1 : 3
            T1 = [rodrigues(omckk) Tckk; 0 0 0 1];
            gridHomoi = [X_kk(:,cubeIdx{j})];
            if j == 1
                T = T1;
            elseif j == 2
                T = T1*config.rt2;
           ...
           [x,dxdom,dxdT] = project_points2(gridHomoi,rodrigues(T(1:3,1:3)),T(1:3,4),fc,cc,kc,alpha_c);
