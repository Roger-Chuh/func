...
plane_norm_norm2 = Check(0, markerRow, MarkerId, pt3d_show, pt3dL, colorStack);
plane_norm_norm1 = Check(1, markerRow, MarkerId, pt3d_show, pt3dL, colorStack);
% plane_norm_norm2 = Check(0, markerRow, MarkerId, pt3d_show, pt3dL, colorStack);

vanPt_angle = dot(plane_norm_norm1(1:3), plane_norm_norm2(1:3));

planeNorm = cross(plane_norm_norm1(1:3), plane_norm_norm2(1:3));
hori_pt = pt3dL(1:3,:);
vert_pt = pt3dL(1:3:end,:);

hori_dir = (hori_pt(1,:) - hori_pt(2,:))./norm(hori_pt(1,:) - hori_pt(2,:));
vert_dir = (vert_pt(1,:) - vert_pt(2,:))./norm(vert_pt(1,:) - vert_pt(2,:));
planeNorm_grid = cross(hori_dir', vert_dir');

planeNorm_grid' - planeNorm'
figure(10),hold on; plotQuiver(planeNorm_grid(1:3)','y'); plotQuiver(-planeNorm_grid(1:3)','y');

planeNorm' - Marker2CamL(1:3,3)'
Marker2CamL(1:3,1:3)
plane_norm_norm
