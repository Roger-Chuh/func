function ProcessCubeDetections2()
...
elseif 0
    ...
    img = imread('C:\vs\test\ConsoleApplication2\ConsoleApplication2\13\marker.png');
...
idOffset = 10000;
coordOffset = 30; 1.234; 0;-48.123; -50; 1.234; 120; -50; 1.234; 0; -50; 1000; 0; 1.234; 50;
...
if ~bigFace
    if ~midFace
        ...
        if 0
            ...
            faceRow = 21;
            faceCol = 30;
            startBottomRowId = [0 32 64];  % [0 15 30];
...
redFaceMat_ = genFaceData(startBottomRowId(1), rowIdStep,faceNum);
greenFaceMat_ = genFaceData(startBottomRowId(2), rowIdStep,faceNum);
blueFaceMat_ = genFaceData(startBottomRowId(3), rowIdStep,faceNum);
if 0
    blueFaceMat_ = blueFaceMat_ + 0;
else
    % 20220312 resolve realsense calibration board two face bug
    blueFaceMat_ = blueFaceMat_ + 20000;
end
redFaceMat = redFaceMat_(end-faceRow+1:end,:);
greenFaceMat = greenFaceMat_(end-faceRow+1:end,:);
blueFaceMat = blueFaceMat_(end-faceRow+1:end,:);
...
if ~wrongOrder
    marker2 = flipud(flipud((rotx(-90)*rotz(-90)*marker1')'));
    marker3 = flipud(flipud((roty(90)*rotz(90)*marker1')'));
else
    marker3 = flipud(flipud((rotx(-90)*rotz(-90)*marker1')'));
    marker2 = flipud(flipud((roty(90)*rotz(90)*marker1')'));
end
if ~wrongRt
...
if ~wrongOrder
    marker22 = flipud(flipud((rotx(90)*rotz(-90)*marker11')'));
    marker33 = flipud(flipud((roty(-90)*rotz(90)*marker11')'));
else
    marker33 = flipud(flipud((rotx(90)*rotz(-90)*marker11')'));
    marker22 = flipud(flipud((roty(-90)*rotz(90)*marker11')'));

    if 1
        marker22_ = flipud(flipud((roty(-70)*rotz(90)*marker11')'));
        marker22_ = flipud(flipud((roty(120)*rotz(0)*marker11')'));
        theta = 90; 90.01;
        if 0
            marker22_ = flipud(flipud((rotx(-110)*rotz(0)*marker11')'));
        else
            if 0
                marker22__ = flipud(flipud((rotx(120)*rotz(0)*marker11')'));
                center_coord = mean(marker22__,1);
                marker22_ = (roty(180)*marker22__')';
            else
                
                marker22__ = flipud(flipud((rotx(theta)*rotz(0)*marker11')'));
                center_coord = mean(marker22__,1);
                marker22_ = (roty(180)*marker22__')';
%                 marker22_ = flipud(flipud((roty(180)*rotx(120)*rotz(0)*marker11')'));
                marker22_ = (((roty(180)*rotx(theta)*rotz(0)*marker11')'));
            end
            %marker22_(:,1) = marker22_(:,1) + max(marker11(:,1));
            marker22_(:,1) = marker22_(:,1) + markerSize*(markerCol-1) + 2*coordOffset + 2*markerSize;
%             marker22_(:,1) = marker22_(:,1) + max(marker11(:,1)) + 2*coordOffset;
%             marker22_(:,1) = marker22_(:,1) + max(marker11(:,1)) + 1*coordOffset + markerSize;
            
        end
        rtIn21__ = AlignCoord(marker22_,marker11);
        tempR = rotx(theta);
        tempR(3,:) = -tempR(3,:);
        tempR(1,1) = -tempR(1,1);
        rtIn21_ = eye(4);
        rtIn21_(1:3,1:3) = tempR;
        rtIn21_(1,4) = markerSize*(markerCol-1) + 2*coordOffset + 2*markerSize;
        assert(            sum(   abs(         rtIn21__(:) - rtIn21_(:)         )   ) < 0.00001         );
        
        figure,pcshow(marker11,[1 0 0]);hold on;pcshow(marker22_,[0 1 0]);
        plot3(marker11(:,1),marker11(:,2),marker11(:,3),'-or');plot3(marker11(1,1),marker11(1,2),marker11(1,3),'sr','MarkerSize',10,'LineWidth', 10);
        plot3(marker22_(:,1),marker22_(:,2),marker22_(:,3),'-og');plot3(marker22_(1,1),marker22_(1,2),marker22_(1,3),'*g','MarkerSize',10,'LineWidth', 10);
...
%         fid1 = fopen(fullfile(inputDir,'gridMid.yaml'),'w');
        fid1 = fopen(fullfile(inputDir,'gridNew.yaml'),'w');
    end
else
...
if 1
    ...
    FaceIdMat = fliplr(reshape(FaceIds, faceNum, [])');
end
